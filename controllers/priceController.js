...
/* ------------------------------------------------------------------
   MIDNIGHT JOB (RESET TODAY DIFF)
------------------------------------------------------------------- */
schedule.scheduleJob("0 0 * * *", async () => {
  try {
    console.log("[JOB] Resetting daily prices...");

    const prices = await Price.find();

    for (const p of prices) {
      const newLast =
        typeof p.currentFinalPrice === "number" && !isNaN(p.currentFinalPrice)
          ? p.currentFinalPrice
          : p.basePrice;
    ...
    console.error("[JOB ERROR] Midnight reset error:", err);
  }
});
  ...
  try {
    ...
    const data = prices.map((p) => {
      const final =
        typeof p.currentFinalPrice === "number" && !isNaN(p.currentFinalPrice)
          ? p.currentFinalPrice
          : typeof p.lastFinalPrice === "number" && !isNaN(p.lastFinalPrice)
          ? p.lastFinalPrice
          : typeof p.todayDiff === "number" && !isNaN(p.todayDiff)
          ? p.basePrice + p.todayDiff
          : typeof p.difference === "number" && !isNaN(p.difference)
          ? p.basePrice + p.difference
          : p.basePrice;

      return { ...p._doc, finalPrice: final };
    });
...
exports.getWebsitePrices = async (req, res) => {
  try {
    ...
    const prices = await Price.find({
      validTill: { $exists: true, $ne: null, $gte: today },
    })
      .populate("category", "name")
      .populate("subcategory", "name")
      .sort({ createdAt: -1 });
...
/* ------------------------------------------------------------------
   UPDATE PRICE (NO DIFF CHANGES)
------------------------------------------------------------------- */
exports.updatePrice = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, category, subcategory, validTill, description } = req.body;
