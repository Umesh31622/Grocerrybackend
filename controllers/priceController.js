    ...
    Readable.from(bufferString)
      ...
      .on("end", async () => {
        ...
        for (const r of results) {
          ...
          const difference = parseFloat(r.difference || 0);
          const validTill = r.validTill ? new Date(r.validTill) : undefined;
          const description = r.description || "";

          let categoryDoc = await Category.findOne({ name: catName });
          if (!categoryDoc) categoryDoc = await Category.create({ name: catName });
        ...
        const populated = await Price.find({
          _id: { $in: createdDocs.map((x) => x._id) },
